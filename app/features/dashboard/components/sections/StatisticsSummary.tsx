...
/**
 * คอมโพเนนต์แสดงสถิติรวม (OPD 24hr, Old Patient, New Patient, Admit 24hr)
 */
const StatisticsSummary: React.FC<StatisticsSummaryProps> = ({ 
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((item) => (
          <div key={item} className="bg-gray-200 dark:bg-gray-700 rounded-xl p-4 shadow-sm animate-pulse h-24">
  ...
  const hasData = Object.values(totalStats).some(val => val !== 0);

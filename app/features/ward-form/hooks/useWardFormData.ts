...
const initialFormStructure: Partial<WardForm> = {
  ...
  initialPatientCensus: undefined, // จำนวนผู้ป่วยเริ่มต้น (ก่อนคำนวณ)
  calculatedCensus: undefined, // จำนวนผู้ป่วยที่คำนวณได้
...
export const useWardFormData = ({
    ...
    setFormData(prevData => {
      ...
      if (selectedShift === ShiftType.NIGHT && 
        ...
        if (isNaN(baseNightCensusForCalculation)) {
            ...
            baseNightCensusForCalculation = 0; // แก้ไขจากเดิมที่ดักเฉพาะ log แล้ว return แต่ไม่ได้แก้ค่า NaN
            setIsFormDirty(true);
            console.log("[handleChange - NaN detected] Corrected NaN to 0");
            // Do not return early, continue with the calculation using 0 as the base
        }

        // Use values from newData for fields NOT being currently changed, and processedValue for the one that is.
        // ปรับการคำนวณเพื่อป้องกัน NaN
        ...
        const nightAdmissions = 
          safeNumber(name === 'newAdmit'   ? processedValue : newData.newAdmit) +
          safeNumber(name === 'transferIn' ? processedValue : newData.transferIn) +
          safeNumber(name === 'referIn'    ? processedValue : newData.referIn);

        const nightDischarges = 
          safeNumber(name === 'discharge'  ? processedValue : newData.discharge) +
          safeNumber(name === 'transferOut'? processedValue : newData.transferOut) +
          safeNumber(name === 'referOut'   ? processedValue : newData.referOut) +
          safeNumber(name === 'dead'       ? processedValue : newData.dead);
        ...
        if (name !== 'patientCensus') {
            ...
            if (isNaN(actualBaseForCalculation)) {
                ...
                actualBaseForCalculation = 0; // เมื่อเป็น NaN ให้ใช้ 0 เป็นค่าเริ่มต้น
            }
            ...
            // สำหรับการคำนวณที่ถูกต้อง ให้บันทึกค่าเริ่มต้นและค่าที่คำนวณได้ด้วย
            newData.initialPatientCensus = actualBaseForCalculation;
            newData.calculatedCensus = calculatedNightCensus;
  ...
  const prepareDataForSave = async (finalSave: boolean = false): Promise<Partial<WardForm> | null> => {
    ...
    if (!wardName && selectedBusinessWardId) {
      // กรณีไม่มี wardName ให้ใช้ wardId
      wardName = selectedBusinessWardId;
    }

    const currentFormData = { ...formData };
    const numericFields: (keyof WardForm)[] = [
      'patientCensus', 'initialPatientCensus', 'calculatedCensus', // เพิ่มฟิลด์ใหม่
      'nurseManager', 'rn', 'pn', 'wc',
      'newAdmit', 'transferIn', 'referIn', 'transferOut', 'referOut',
      'discharge', 'dead', 'available', 'unavailable', 'plannedDischarge'
    ];

    // ตรวจสอบว่ามีการคำนวณ calculatedCensus หรือไม่
    if (currentFormData.calculatedCensus === undefined) {
      // คำนวณค่า calculatedCensus
      const admissions = (
        (currentFormData.newAdmit || 0) +
        (currentFormData.transferIn || 0) + 
        (currentFormData.referIn || 0)
      );
      const discharges = (
        (currentFormData.discharge || 0) + 
        (currentFormData.transferOut || 0) + 
        (currentFormData.referOut || 0) + 
        (currentFormData.dead || 0)
      );
      
      const initialValue = currentFormData.initialPatientCensus !== undefined ? currentFormData.initialPatientCensus : (currentFormData.patientCensus || 0);
      currentFormData.calculatedCensus = Math.max(0, initialValue + admissions - discharges);
    }

    // ตรวจสอบว่ามีค่า initialPatientCensus หรือไม่
    if (currentFormData.initialPatientCensus === undefined && currentFormData.patientCensus !== undefined) {
      currentFormData.initialPatientCensus = currentFormData.patientCensus;
    }
    ...
    const data: Partial<WardForm> = {
      ...
      wardId: selectedBusinessWardId,
      wardName: wardName,
      date: Timestamp.fromDate(new Date(selectedDate + 'T00:00:00')), // แก้ไขให้เป็น Timestamp แทน Date
      shift: selectedShift,
    ...
    // แปลงค่าในฟิลด์ที่เป็นตัวเลขให้เป็น number
    numericFields.forEach((field) => {
      if (data[field] !== undefined) {
        // แก้ไขให้ใช้ type assertion เพื่อหลีกเลี่ยง TypeScript error
        (data as any)[field] = Number(data[field]);
      }
    });

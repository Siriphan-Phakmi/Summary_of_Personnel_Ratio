...
### แก้ไขโครงสร้างโค้ด Dashboard ให้มีประสิทธิภาพและความปลอดภัยมากขึ้น
- [x] **รวมและจัดระเบียบ Interface ที่ซ้ำซ้อน**:
  - [x] รวม interface ที่ซ้ำซ้อนระหว่างไฟล์ให้เหลือเพียงไฟล์เดียว
  - [x] ปรับปรุงการใช้ `export type` ให้ถูกต้องตามมาตรฐาน TypeScript และ `isolatedModules`
  - [x] แก้ไขการนำเข้า (import) interface ต่างๆ ให้มีประสิทธิภาพและไม่ซ้ำซ้อน

- [x] **ปรับปรุง Index Files และการส่งออกโมดูล**:
  - [x] เพิ่ม export ให้กับไฟล์ index.ts ที่ว่างเปล่าทั้งหมด
  - [x] ปรับปรุงการส่งออกคอมโพเนนต์ให้ใช้งานง่ายและเป็นระเบียบมากขึ้น
  - [x] แก้ไขการ export ชื่อซ้ำกันในโมดูลต่างๆ

- [x] **ปรับปรุง Props ให้ตรงกับ Interface**:
  - [x] แก้ไข interface `PatientTrendChartProps` ให้รองรับ method `handleWardChange`
  - [x] ตรวจสอบและปรับปรุง props ของคอมโพเนนต์อื่นๆ ให้ตรงกับ interface ที่ประกาศไว้

- [x] **เพิ่ม Input Validation**:
  - [x] เพิ่มการตรวจสอบข้อมูลเข้าใน `calculatePatientCensus` เพื่อป้องกันข้อมูลที่ไม่ถูกต้อง
  - [x] เพิ่มการตรวจสอบข้อมูลเข้าใน `formatChartData` เพื่อป้องกันการ crash เมื่อข้อมูลไม่สมบูรณ์
  - [x] เพิ่มการตรวจสอบข้อมูลเข้าใน `hasDataForDate` เพื่อความปลอดภัยและเสถียรภาพ

**เหตุผลในการแก้ไข:**
- **ลดความซ้ำซ้อน**: การรวม interface ที่ซ้ำซ้อนช่วยลดปัญหาความไม่สอดคล้องของข้อมูลและทำให้โค้ดอ่านง่ายขึ้น
- **เพิ่มความปลอดภัย**: การตรวจสอบข้อมูลเข้าช่วยป้องกันข้อผิดพลาดที่อาจเกิดขึ้นจากข้อมูลที่ไม่ถูกต้อง
- **เพิ่มความเสถียร**: การแก้ไขโครงสร้างโค้ดช่วยให้แอปพลิเคชันทำงานได้อย่างเสถียรและมีประสิทธิภาพมากขึ้น

**ผลกระทบที่อาจเกิดขึ้น:**
- การเปลี่ยนแปลงวิธีการ export/import อาจต้องมีการปรับปรุงโค้ดในส่วนอื่นๆ ที่เกี่ยวข้อง
- การเพิ่มการตรวจสอบข้อมูลอาจทำให้โค้ดทำงานช้าลงเล็กน้อย แต่ได้ความปลอดภัยที่เพิ่มขึ้น

**ข้อควรระวัง:**
- ควรทดสอบการทำงานของแอปพลิเคชันหลังการแก้ไขอย่างละเอียด โดยเฉพาะส่วนที่มีการแก้ไขโครงสร้าง
- ควรตรวจสอบการทำงานของโค้ดกับข้อมูลในหลายรูปแบบ ทั้งข้อมูลปกติและข้อมูลที่อาจไม่สมบูรณ์